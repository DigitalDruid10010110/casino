...
class NPCPlayer:
    ...
    def make_decision(self, hand_index=0):
        """Decide whether to hit, stand, or split, with a dash of randomness."""
        if self.current_game == "Blackjack":
            ...
            if self.can_split(hand_index):
                decision = random.choice(["split", "hit", "stand"])  # Randomly choose between splitting, hitting, or standing
                if decision == "split":
                    self.split_hand()
                    return "split"
